...
- A file `./.github/changelog-config.json` must exist in the repository. This file can be copied and adjusted from [here](https://github.com/box-id/github-actions-oss/blob/main/.github/changelog-config.json)
...
- A file `./.github/changelog-config.json` must exist in the repository. This file can be copied and adjusted from [here](https://github.com/box-id/github-actions/blob/main/.github/changelog-config.json)
...
jobs:
  create_version_bump_pr:
    name: Create Version Bump PR
    uses: box-id/github-actions/.github/workflows/npm_version_bump.yml@main
    with:
      version-prefix: "v" # optional, defaults to "v"
    secrets: inherit
```
...
If the workflow detects no change between the version in `package.json` and the last-published GitHub release, it will **not** create a release and publish to NPM.
...
jobs:
  release:
    name: Publish a release
    uses: box-id/github-actions/.github/workflows/npm_publish_release.yml@main
    with:
      version-prefix: "v" # optional, defaults to "v"
    secrets: inherit
```
